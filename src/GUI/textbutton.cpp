...
TextButton::TextButton(const std::string text, const int x, const int y, sdl::Renderer& renderer, std::function<void(Ui* ui)> callback_function, const TextButtonKind textbutton_kind)
	: Ui(renderer), 
	  text_(text, constants::textbutton_text_color_, constants::textbutton_font_, textbutton_kind == TextButtonKind::ON_TEXTBOX ? constants::textbox_textbuttons_text_size_ : textbutton_kind == TextButtonKind::ON_FRAME ? constants::confirmationpopup_text_size_ : constants::textbutton_text_size_, x, y, renderer)
{
	callback_function_ = callback_function;
	pointer_on_ui_when_pointer_up_ = true;
}

TextButton::TextButton(const std::string text, const int x, const int y, sdl::Renderer& renderer, const std::string text_popup, std::function<void(Ui* ui)> callback_function, const TextButtonKind textbutton_kind)
	: Ui(renderer),
	text_(text, constants::textbutton_text_color_, constants::textbutton_font_, textbutton_kind == TextButtonKind::ON_TEXTBOX ? constants::textbox_textbuttons_text_size_ : textbutton_kind == TextButtonKind::ON_FRAME ? constants::confirmationpopup_text_size_ : constants::textbutton_text_size_, x, y, renderer),
