	...
	: CheckableGroup(number_of_toggles, only_one_has_to_be_checked, top_text, x, y, renderer) 
{
	...
	for(unsigned int i = 0; i < number_of_toggles; ++i)
	{
		...
		this->checkables.push_back(std::make_unique<TextToggle>(texts[i], color_unchecked, color_selected, color_checked, x, y + this->y_spacing_between_checkable * i, is_checked, renderer, callback_functions[i]));
