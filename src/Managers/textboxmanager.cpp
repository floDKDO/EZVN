...
TextboxManager::TextboxManager(sdl::Renderer& renderer, Game& game)
	: last_time_(0), which_dialogue_from_where_({Where::none, false, false}), skip_mode_(false), auto_mode_(false), hide_ui_textbox_(false), where_("bottom"), //TODO : hardcodé
...
{
	std::unique_ptr<Ui> history_ui = std::make_unique<TextButton>("History", 0, 0, renderer, std::bind(&TextboxManager::temp_function, this, std::placeholders::_1), TextButtonKind::ON_TEXTBOX);
	history_button_ = dynamic_cast<TextButton*>(history_ui.get());
	ui_manager_.add_element(std::move(history_ui));

	std::unique_ptr<Ui> skip_ui = std::make_unique<TextToggle>("Skip", 0, 0, false, renderer, std::bind(&TextboxManager::skip_function, this, std::placeholders::_1), TextToggleKind::ON_TEXTBOX);
	skip_toggle_ = dynamic_cast<TextToggle*>(skip_ui.get());
	ui_manager_.add_element(std::move(skip_ui));

	std::unique_ptr<Ui> auto_ui = std::make_unique<TextToggle>("Auto", 0, 0, false, renderer, std::bind(&TextboxManager::auto_function, this, std::placeholders::_1), TextToggleKind::ON_TEXTBOX);
	auto_toggle_ = dynamic_cast<TextToggle*>(auto_ui.get());
	ui_manager_.add_element(std::move(auto_ui));

	std::unique_ptr<Ui> save_ui = std::make_unique<TextButton>("Save", 0, 0, renderer, std::bind(&TextboxManager::temp_function, this, std::placeholders::_1), TextButtonKind::ON_TEXTBOX);
	save_button_ = dynamic_cast<TextButton*>(save_ui.get());
	ui_manager_.add_element(std::move(save_ui));

	std::unique_ptr<Ui> load_ui = std::make_unique<TextButton>("Load", 0, 0, renderer, std::bind(&TextboxManager::temp_function, this, std::placeholders::_1), TextButtonKind::ON_TEXTBOX);
	load_button_ = dynamic_cast<TextButton*>(load_ui.get());
	ui_manager_.add_element(std::move(load_ui));

	std::unique_ptr<Ui> settings_ui = std::make_unique<TextButton>("Settings", 0, 0, renderer, std::bind(&TextboxManager::settings_function, this, std::placeholders::_1), TextButtonKind::ON_TEXTBOX);
