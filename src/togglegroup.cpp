...
ToggleGroup::ToggleGroup(unsigned int number, std::string text, const std::string path_normal, const std::string path_selected, const std::string path_clicked, const std::string path_checked, const int x, const int y, bool is_checked, SDL_Renderer* renderer, std::function<void(Ui* ui)> callback_function)
	: number(number), text(text, {255, 255, 255, 255}, "fonts/Aller_Rg.ttf", 50, x, y - 100, renderer), only_one_has_to_be_checked(true) //TODO : paramètre du constructeur
{
	...
	for(unsigned int i = 0; i < number; ++i)
	{
		...
		Toggle* toggle_i = new Toggle(path_normal, path_selected, path_clicked, path_checked, x, y + 200 * i, is_checked, renderer, callback_function);
