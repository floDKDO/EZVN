...
class TransformStep
{
	public:
		...
		void rotate(Image& image, double angle, Uint64 time = 0);

		void zoom(Image& image, float zoom, Uint64 time = 0);
		void resize(Image& image, int w, int h, Uint64 time = 0);

		void set_position_common(Image& image, int x, int y, Uint64 time = 0);
		void set_position(Image& image, int x, int y, Uint64 time = 0);
		void set_position_xcenter(Image& image, int x, Uint64 time = 0);
		void set_position_ycenter(Image& image, int y, Uint64 time = 0);
	    void set_position_xycenter(Image& image, int x, int y, Uint64 time = 0);

		void set_position_xoffset(Image& image, int x, Uint64 time = 0);
		void set_position_yoffset(Image& image, int y, Uint64 time = 0);

		void set_center(Image& image, Uint64 time = 0);

		void filter_common(Image& image, Uint8 r, Uint8 g, Uint8 b, Uint64 time = 0);
		void night_filter(Image& image, Uint64 time = 0);
		void afternoon_filter(Image& image, Uint64 time = 0);
		void own_filter(Image& image, Uint8 r, Uint8 g, Uint8 b, Uint64 time = 0);
	...
	private:
		...
		{
			public:
				PositionStep(int initial_position_x, int initial_position_y, int x, int y)
		...
		struct SizeStep //resize(), zoom()
		{
			public: 
				//for zoom()
				SizeStep(int initial_size_w, int initial_size_h, float zoom)
				...
				//for resize()
				SizeStep(int initial_size_w, int initial_size_h, int w, int h)
					: initial_size_w_(initial_size_w), initial_size_h_(initial_size_h),
					f_size_w_(float(initial_size_w)), f_size_h_(float(initial_size_h)),
		...
		{
			public:
				RotateStep(double initial_angle, double angle)
		...
		{
			public:
				AlphaStep(int initial_alpha, int alpha)
		...
		{
			public:
				FilterStep(int initial_r, int initial_g, int initial_b, int r, int g, int b)
